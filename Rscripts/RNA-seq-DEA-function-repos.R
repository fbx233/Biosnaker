...
volcano_plot<-function(res,output_name,plot_title,xlab,ylab){
  ...
  non_na_values <- res$log2FoldChange[!is.na(res$log2FoldChange)]
  x_range <- max(abs(c(min(non_na_values), max(non_na_values))))
  ...
  p12 <- p12 + scale_x_continuous(limits = c(-x_range, x_range))
...
Differential_expression_analysis<-function(input_files,sample_info_path,comparisons,refgroup,output_name,human_metabolic_gene,plot_title,xlab,ylab,padjThread=0.05,readssumThread=1,L2FCThread=1.5){
  # Read the input data
  raw_count<-read.table(input_files,sep='\t',header=T,check.names = FALSE)
...
KEGG_enrich_plot<-function(kegg_table,output_name){
  ...
  pdf(paste0(output_name,"KEGG-Enrichmet-plot.pdf"),width=12,height=12)
  print(p1)
  dev.off()
}
...
KEGG_enrich_metabolism_related_plot<-function(kegg_table,output_name,pathway_list_path){
  ...
  p1<-ggplot(kegg_table,aes(x=-log10(p.adjust),y=Description))+
    ...
    theme(plot.title = element_text( size = 16, vjust = 6, hjust = 0.5, face = "plain"), 
          ...
          legend.key.size = unit(0.4, "cm"), 
          legend.spacing = unit(0.4, "cm"))
  pdf(paste0(output_name,"Metabolism-related-KEGG-Enrichmet-plot.pdf"),width=12,height=12)
  print(p1)
  dev.off()
}
...
multi_group_KEGG_enrich_plot <- function(mg_kegg_enrich, output_name) {
  ...
  if (all(is.na(mg_kegg_enrich$FoldEnrich))) {
    warning("FoldEnrich column contains only NA values. Please check your data.")
    return(NULL)
  }

  if (all(is.na(mg_kegg_enrich$p.adjust))) {
    warning("p.adjust column contains only NA values. Please check your data.")
    return(NULL)
  }
  
  # Filter out rows with NA values in FoldEnrich or p.adjust
  mg_kegg_enrich <- mg_kegg_enrich %>%
    filter(!is.na(FoldEnrich) & !is.na(p.adjust))

  # Calculate the -log10FDR and the range of FoldEnrich for KEGG_enrich_plot.
  min_log10FDR <- floor(min(-log10(mg_kegg_enrich$p.adjust), na.rm = TRUE))
  max_log10FDR <- ceiling(max(-log10(mg_kegg_enrich$p.adjust), na.rm = TRUE))
  min_FoldEnrich <- floor(min(mg_kegg_enrich$FoldEnrich, na.rm = TRUE))
  max_FoldEnrich <- ceiling(max(mg_kegg_enrich$FoldEnrich, na.rm = TRUE))
  
  # Initialize the ggplot object
  p2 <- mg_kegg_enrich %>%
    ggplot(aes(x = cluster, y = reorder(Description, -p.adjust))) +
    geom_point(aes(fill = -log10(p.adjust), size = FoldEnrich), shape = 21, color = "grey40") +
    scale_fill_viridis_c(direction = -1, end = 0.9, option = "C", limit = c(min_log10FDR, max_log10FDR)) +
    ...
    theme(panel.grid.major.x = element_line(size = 0), 
          panel.grid.minor.x = element_line(size = 0),
          plot.title = element_text(size = 32, vjust = 6, hjust = 0.5, face = "plain"),
          axis.title.x = element_text(size = 32, vjust = -2, hjust = 0.55, face = "plain"),
          axis.title.y = element_text(size = 32, vjust = 2, face = "plain"),
          axis.text.x = element_text(size = 32, face = "plain", colour = "black", angle = 90, vjust = 0.5, hjust = 1),  # Rotate x-axis labels
...
multi_group_KEGG_enrich_plot_metabolic_pathway<-function(mg_kegg_enrich,output_name,pathway_list_path){
  # Filter out the metabolic pathways among them
  pathway_list<-read.csv(pathway_list_path)
  Metabolism_pathway_list<-pathway_list%>%filter(Class_III=="Metabolism")
  mg_kegg_enrich<-mg_kegg_enrich%>%filter(ID %in% c(Metabolism_pathway_list$ID))
  # Calculate the -log10FDR and the range of FoldEnrich for KEGG_enrich_plot.
  ...
  plot_height = ceiling(length(unique(mg_kegg_enrich$Description)) / 30) *10
  ...
  pdf(paste0(output_name,"KEGG-Enrichment-plot_meabolic_pathway.pdf"),width=30,height=plot_height)
  print(p2)
  dev.off()
}
...
multi_group_KEGGpathwayEnrich<-function(input_files,sample_info_path,
                                        ...
                                        human_metabolic_gene,plot_title,xlab,ylab,pathway_list_path,
  ...
  for(key in names(comparisons_dir)){
    ...
    allNmr<-Differential_expression_analysis(input_files,sample_info_path,
                                             ...
                                             padjThread=padjThread,readssumThread=readssumThread,L2FCThread=L2FCThread)
    all_gene<-allNmr$all
    mr_gene<-allNmr$mr
    ...
    # Perform KEGG enrichment for all genes
    all_kegg_enrich<-KEGGpathwayEnirch(gene_set=all_DE_gene$SYMBOL,fromType="SYMBOL",toType="ENTREZID",OrgDb="org.Hs.eg.db",organism="hsa",pvalueCutoff = 0.05,qvalueCutoff = 0.05)
    write.csv(all_kegg_enrich,paste0(output_name,"-KEGG-Enrichment-Result-All-gene.csv"),row.names = F)
    # Perform KEGG enrichment for metabolism-related genes
    mr_kegg_enrich<-KEGGpathwayEnirch(gene_set=mr_DE_gene$SYMBOL,fromType="SYMBOL",toType="ENTREZID",OrgDb="org.Hs.eg.db",organism="hsa",pvalueCutoff = 0.05,qvalueCutoff = 0.05)
    write.csv(mr_kegg_enrich,paste0(output_name,"-KEGG-Enrichment-Result-mr-gene.csv"),row.names = F)
  ...
  write.csv(all_kegg_enrich_combined,paste0(plot_name,"-KEGG-Enrichment-Result-All-gene-combined.csv"),row.names = F)
  mr_kegg_enrich_combined <- do.call("rbind", mr_kegg_enrich_list)
  write.csv(mr_kegg_enrich_combined,paste0(plot_name,"-KEGG-Enrichment-Result-mr-gene-combined.csv"),row.names = F)
  # Example calculation of FoldEnrich (modify as needed for your specific data)
  all_kegg_enrich_combined$FoldEnrich <- with(all_kegg_enrich_combined, Count / (as.numeric(strsplit(GeneRatio, "/")[[1]][1]) / as.numeric(strsplit(BgRatio, "/")[[1]][1])))
  # Replace NA with 0 if appropriate
  all_kegg_enrich_combined$FoldEnrich[is.na(all_kegg_enrich_combined$FoldEnrich)] <- 0
  mr_kegg_enrich_combined$FoldEnrich <- with(mr_kegg_enrich_combined, 
    (as.numeric(sapply(strsplit(GeneRatio, "/"), `[`, 1)) / 
    as.numeric(sapply(strsplit(GeneRatio, "/"), `[`, 2))) / 
    (as.numeric(sapply(strsplit(BgRatio, "/"), `[`, 1)) / 
    as.numeric(sapply(strsplit(BgRatio, "/"), `[`, 2)))
    )
  mr_kegg_enrich_combined$FoldEnrich[is.na(mr_kegg_enrich_combined$FoldEnrich)] <- 0  
  ...
  # Draw bubble plots for multiple sets of KEGG enrichment with metabolic pathway.

  multi_group_KEGG_enrich_plot_metabolic_pathway(all_kegg_enrich_combined,paste0(plot_name,"-all-"),pathway_list_path)
  multi_group_KEGG_enrich_plot_metabolic_pathway(mr_kegg_enrich_combined,paste0(plot_name,"-mr-"),pathway_list_path)
